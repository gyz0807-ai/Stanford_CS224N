...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        batch_parsers = unfinished_parses[:batch_size]
        next_trans = model.predict(batch_parsers)
        for trans, p_parse in zip(next_trans, batch_parsers):
            p_parse.parse([trans])
            if (not p_parse.buffer) and (len(p_parse.stack) == 1):
                unfinished_parses.remove(p_parse)
